...
/*!
 * Determine if two strings are equal without regard to case (for ASCII
 * characters).
 * \param p is the first string to compare.
 * \param q is the second string to compare.
 * \param nChar is the maximum number of characters to examine.
 * \return true if the strings are equal (up to nChar), else false.
 */
bool XMLUtil::_StringEqual( const char* p, const char* q, int nChar )
{
    int i = 0;
    ...
    while(i < nChar)
    {
        ...
        // Any character which is in the range of 'a' to 'z' is converted to
        // uppercase 'A' to 'Z'. Note that any UTF-8 character will have a
        // leading 1 bit, so will not be converted.
        if((one >= 'A') && (one <= 'Z'))
            one += (char)('a' - 'A');
        ...
        // If the characters are not the same, the strings are different. Else
        // if they are identical and zero the strings are finished and the same
        if(one != two)
            return false;
        else if(one == 0)
            return true;
    ...
    // If we get here it is because we hit our character limit without finding
    // 0, but also without finding a difference up to nCha.r
    return true;

}// XMLUtil::_StringEqual
